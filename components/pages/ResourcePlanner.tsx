...
export const ResourcePlanner = () => {
  const { navigateTo, users, tasks, roles, groups, addNotification, setToolbarConfig } = useBPM();
  ...
  const scheduleData = useMemo(() => {
      return filteredUsers.map(u => {
          ...
          const userRoleIds = u.roleIds || [];
          const userGroupIds = u.groupIds || [];

          const roleName = roles.find(r => userRoleIds.includes(r.id))?.name || 'Staff';
          const groupName = groups.find(g => userGroupIds.includes(g.id))?.name || 'General';
    ...
    <div className="animate-fade-in flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 -mx-4 px-4 pb-10">
            <PageGridLayout defaultLayouts={defaultLayouts}>
                {({ isEditable }) => (
                    <NexCard key="grid" dragHandle={isEditable} className="p-0 overflow-hidden flex flex-col shadow-sm">
                        {/* Header Row */}
                        <div className="grid grid-cols-12 bg-subtle border-b border-default shrink-0">
                            <div className="col-span-3 text-[10px] font-bold text-secondary uppercase tracking-wider flex items-center p-03">Principal</div>
                            <div className="col-span-1 text-center text-[10px] font-bold text-secondary uppercase tracking-wider border-l border-default flex items-center justify-center p-03">W/L</div>
                            <div className="col-span-8 flex border-l border-default">
                                {days.map((d, i) => {
                                    const isToday = d.toDateString() === new Date().toDateString();
                                    return (
                                        <div key={i} className={`flex-1 text-center border-r border-default last:border-0 flex flex-col justify-center p-02 ${isToday ? 'bg-blue-50/50' : ''}`}>
                        ...
                        <div className="divide-y divide-default overflow-y-auto flex-1">
                            {scheduleData.length === 0 ? (
                                <div className="text-center text-tertiary italic text-xs p-05">No resources match current filter.</div>
                            ) : (
                                scheduleData.map((row) => (
                                    <div key={row.user.id} className="grid grid-cols-12 items-stretch hover:bg-subtle transition-colors group min-h-[60px]">
                                        {/* User Info */}
                                        <div className="col-span-3 flex items-center p-03 gap-03">
                                            <div 
                                                className={`w-9 h-9 rounded-base flex items-center justify-center text-xs font-bold shrink-0 border cursor-pointer transition-transform hover:scale-105 ${row.user.status === 'Active' ? 'bg-slate-800 text-white border-slate-900' : 'bg-slate-100 text-slate-400 border-dashed border-slate-300'}`}
                                                onClick={() => addNotification('info', `Resource details for ${row.user.name} are managed under Identity & Governance.`)}
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <div className="text-[10px] text-secondary truncate flex items-center gap-01">
                                        ...
                                        <div className="col-span-8 flex border-l border-default">
                                            {row.dailyLoad.map((dayData, i) => {
                                                ...
                                                // LIST MODE (Standard)
                                                return (
                                                    <div key={i} className={`flex-1 border-r border-default last:border-0 relative ${isToday ? 'bg-blue-50/30' : ''}`} style={{ padding: 'var(--spacing-02)' }}>
                                                        {dayData.count > 0 ? (
                                                            <div 
                                                            ...
                                                            >
                                                                ...
                                                                {dayData.hasCritical && <Zap size={8} className="mt-01 animate-pulse"/>}
                                                            </div>
                                                        ) : (
