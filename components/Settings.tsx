...
export const Settings: React.FC = () => {
  ...
  const defaultLayouts = {
      lg: [
          { i: 'system', x: 0, y: 0, w: 6, h: 5 },
          { i: 'appearance', x: 6, y: 0, w: 6, h: 5 },
          { i: 'security', x: 0, y: 5, w: 4, h: 6 },
          { i: 'sso', x: 4, y: 5, w: 4, h: 6 },
          { i: 'compliance', x: 8, y: 5, w: 4, h: 6 },
          { i: 'health', x: 0, y: 11, w: 12, h: 5 }
      ],
      md: [
          { i: 'system', x: 0, y: 0, w: 12, h: 5 },
          { i: 'appearance', x: 0, y: 5, w: 12, h: 5 },
          { i: 'security', x: 0, y: 10, w: 6, h: 6 },
          { i: 'sso', x: 6, y: 10, w: 6, h: 6 },
          { i: 'compliance', x: 0, y: 16, w: 12, h: 5 },
          { i: 'health', x: 0, y: 21, w: 12, h: 5 }
      ]
  };
        ...
        <PageGridLayout defaultLayouts={defaultLayouts}>
            {({ isEditable }) => [
                ...
                <NexCard key="sso" dragHandle={isEditable} title="Global SSO Provider" className="p-6">
                    <div className="space-y-4">
                        {Object.entries(settings.sso || {}).map(([key, val]) => (
                            <div key={key} className="flex items-center justify-between p-2 border border-default rounded-sm bg-subtle">
                                ...
                                <button 
                                    onClick={() => updateSystemSettings({ sso: { ...settings.sso, [key]: !val }})}
                                    className={`w-8 h-4 rounded-full transition-colors relative ${val ? 'bg-blue-600' : 'bg-slate-300'}`}
                                >
                                    <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-transform ${val ? 'left-4.5' : 'left-0.5'}`} />
                                </button>
                            </div>
                        ))}
                    </div>
