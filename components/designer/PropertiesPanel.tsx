...
export const PropertiesPanel = ({ 
  ...
  onClose?: () => void;
    ...
    // 1. Hooks MUST be called at the very top, unconditionally.
    const { rules, forms, navigateTo } = useBPM();
    const [activeTab, setActiveTab] = useState<'config' | 'data' | 'logic' | 'policy'>('config');
    
    // 2. Safe derivation of dependencies for useMemo to avoid conditional access
    const stepType = step?.type;
    const entryAction = step?.onEntryAction;
    const exitAction = step?.onExitAction;

    // 3. Always call useMemo
    const panelWidthClass = useMemo(() => {
        ...
        if (activeTab === 'logic' && (entryAction || exitAction)) return 'w-[450px]';

        // Specific complex steps
        if (['script-task', 'pdf-generate', 'template-render', 'xml-transform', 'json-map', 'api-gateway', 'sql-query', 'graphql-query'].includes(stepType || '')) {
            return 'w-[480px]';
        }
        
        if (['sendgrid-email', 'email-send'].includes(stepType || '')) return 'w-[400px]';

        return 'w-[320px]';
    }, [step, stepType, activeTab, entryAction, exitAction]);

    // 4. Helper derivations
    ...
    // 6. Final Return - Conditional Rendering Logic MUST be here, not before hooks
    return (
