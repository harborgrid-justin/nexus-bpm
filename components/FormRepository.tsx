...
import { Plus, Layout, Edit, Trash2, Copy, FileText, Settings, GripVertical, Search, FilePlus } from 'lucide-react';
import { NexButton, NexCard, NexEmptyState } from './shared/NexUI';
    ...
    <div className="animate-fade-in flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 -mx-4 px-4 pb-10">
            <ResponsiveGridLayout className="layout" layouts={layouts} breakpoints={{ lg: 1200, md: 996, sm: 768, xs: 480, xxs: 0 }} cols={{ lg: 12, md: 10, sm: 6, xs: 4, xxs: 2 }} rowHeight={gridConfig.rowHeight} margin={gridConfig.margin} isDraggable={isEditable} isResizable={isEditable} draggableHandle=".drag-handle" onLayoutChange={(curr, all) => setLayouts(all)}>
                <NexCard key="grid" dragHandle={isEditable} className="p-0 overflow-hidden flex flex-col h-full bg-transparent border-0 shadow-none">
                        ...
                        {forms.length === 0 ? (
                            <div className="flex items-center justify-center h-full">
                                <NexEmptyState 
                                    ...
                                    description="Get started by designing your first data entry form." 
                                    action={<NexButton variant="primary" onClick={() => navigateTo('form-designer')}>Design Form</NexButton>}
                                />
                            </div>
                        ) : (
